...
function Device_UpdateStatus() {
    ...
    if (dDeviceType.options[dDeviceType.selectedIndex].innerText == "DOME_TIANDY") {
        if (lyDeviceCtrl.style.visibility == "visible") { ly3D.style.visibility = "visible"; }
    }
    else { ly3D.style.visibility = "hidden"; }
}
    ...
    try {
        ...
        for (i = 0; i < selNVS.length; i++) {
            ...
            if ((arrUnion[2] == sServerHost) && (sProxyIP == arrUnion[1])) {
                ...
                TiandyVideo.Commander(Obs_ShowMsg_Cmd, iCurObs, true, JS_cShow244, 1000, 0, 0, 0, 0, 0);
                return;
            }
        }
...
////	iChannelNum     =	1;
//    var a = TiandyVideo.Commander(Obs_StartRealTime_Cmd,iCurObs,iCurLogon,0,iStreamNO,20*iDelayNum,iDelayNum,iMode,0,0).split('\n');
//	
////	iCurObs =   TiandyVideo.CurObs;
//	alert("登陆后连接视频"+a);
//}
...
function DHT_StartObs() {
    ...
    var a = TiandyVideo.Commander(Obs_StartRealTime_Cmd,//链接视频命令码
        iCurObs,//当前选中窗口号，默认为0
        iCurLogon,//登陆状态，默认为0
        iChannelNum,//通道号，默认为0
        iStreamNO,//流通道号
        20 * iDelayNum,
...
function LoadHtml() {
    if (jsLanguage == 1) { TiandyVideo.iLanguage = 1; }
    if (AXCOLOR == 0) {
        BACKCOLOR = "#A6DB9E";
    }
    else {
